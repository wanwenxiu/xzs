...
public class FileUtils {
    ...
    public static File getAppDir(Context context) {
        ...
        if (isSdCardExists && isRootDirExists) {
            dirPath = String.format("%s/%s/", Environment.getExternalStorageDirectory().getAbsolutePath(), Contains.ROOT_PATH);
        } else {
            dirPath = String.format("%s/%s/", context.getApplicationContext().getFilesDir().getAbsolutePath(), Contains.ROOT_PATH);
        }
    ...
    public static File getAppRecordDir(Context context) {
        ...
        File recordDir = new File(appDir.getAbsolutePath(), Contains.RECORD_DIR);
        if (!recordDir.exists()) {
            recordDir.mkdir();
        }
        return recordDir;
